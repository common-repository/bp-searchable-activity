=== BP Searchable Activity ===
Contributors: sbrajesh,buddydev,anusharma
Tags: buddypress, activity, search
Requires at least: 4.5
Tested up to: 4.9
Stable tag: 1.0.2
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

BuddyPress Searchable Activity allows users to search BuddyPress Activity

== Description ==
**This plugin is deprecated and we won't be able to provide support for it**
BuddyPress Searchable Activity allows users to search BuddyPress activity. It is just a plug and play plugin.
Just activate it and it will add "Activity" as a drop down in the Site search filter(where you see posts, members etc). 
Please see screenshots for more details.

Special Thanks to [@imath](http://imath.owni.fr/) for making it independent of any js modifications in the theme.

Please leave a comment on our blog [BuddyDev](http://buddydev.com/buddypress/making-buddypress-activity-searchable/) for any support.


== Installation ==

This section describes how to install the plugin and get it working.

e.g.

1. Download the zip file
1. Unzip it
1. Upload `bp-searchable-activity` to the `/wp-content/plugins/` directory
1. Activate the plugin through the 'Plugins' menu in WordPress
1. Go ahead and check your website. You should see the Activity filter in BP search form

== Frequently Asked Questions ==

= No Questions, just solutions =

Ok, we need some question first.



== Screenshots ==

1. Activity Dropdown screenshot-1.png
2. Searched Activity screenshot-2.png

== Changelog ==

= 1.0 =
* Initial release


== Others ==

Please leave a comment on our blog [BuddyDev](http://buddydev.com/buddypress/making-buddypress-activity-searchable/) for any support.
